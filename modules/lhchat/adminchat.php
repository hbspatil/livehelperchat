    	    ...
    	    if ($operatorAccepted == true) {
                ...
                ?>
                   <script>
                    ...
                    xmlhttp1.open("POST", "https://curie.intentico.xyz/maya_multitenant/updateAgentTransfer");
                    xmlhttp1.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
                    xmlhttp1.send(JSON.stringify({ session_id:<?php echo ($chat->id) ;?>,user_id : <?php echo ($chat->user_id) ?>, dep_id: <?php echo($chat->dep_id) ?>, waitingTime: <?php echo($chat->wait_time);  ?>, response: { name: "Tester" } }));
                   </script> 
