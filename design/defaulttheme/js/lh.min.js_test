...
function lh(){
	...
	this.closeActiveChatDialog = function(chat_id, tabs, hidetab)
	{
        ...
        var nickName = $("#messagesBlock-"+chat_id+" span.usr-tit").first().text();
        var nickName1 = nickName.substring(9);
        var xmlhttp1 = new XMLHttpRequest();  
        var chatMessage = "//feed_back_form";
        xmlhttp1.open("POST", "https://lab.intentico.ai/maya_multitenant_test/agentChat");
        xmlhttp1.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
        xmlhttp1.send(JSON.stringify({ session_id:nickName1,message :chatMessage, "agent":true, response: { name: "Tester" } }));
    ...
    this.submitL1andL2ANDCLOSE = function(chat_id, tabs, hidetab, l1 ,l2)
    {
        //var parentSelection  = $('#parent_selection').find('option:selected').val();
        var parentSelection = $('#parent_selection1').find('option:selected').val();
        var subcategorySelection  =  $('#sub_category_div').find('option:selected').val();
        var index = document.getElementById(l1);   

     
        console.log("parentSelection",parentSelection, l1);
        console.log("subcategorySelection",subcategorySelection, l2);
        ...
        xmlhttp1.open("POST", "https://lab.intentico.ai/maya_multitenant_test/agentChat");
        xmlhttp1.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
        xmlhttp1.send(JSON.stringify({ session_id:nickName1,message :chatMessage, "l1":parentSelection, "l2": subcategorySelection,"agent":true, response: { name: "Tester" } }));
	...
	this.addmsgadmin = function (chat_id)
	{
        ...
        console.log("addmsgadmin ",chat_id);
		...
		console.log("pdata ",pdata.msg);
        //console.log("nickName1",nickName1);
       /* var xmlhttp = new XMLHttpRequest();   // new HttpRequest instance 
        xmlhttp.open("GET", "http://support.intentico.xyz/getIntId?q="+chat_id);
        ...
        xmlhttp1.open("POST", "https://lab.intentico.ai/maya_multitenant_test/agentChat");
        xmlhttp1.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
        xmlhttp1.send(JSON.stringify({ session_id:nickName1,message :pdata.msg, response: { name: "Tester" } }));
	...
	this.afterAdminChatInit = function (chat_id) {
        if (LHCCallbacks.afterAdminChatInit) {
            console.log("afterAdminChatInit");
    ...
    this.addFileUpload = function(data_config) {
    	...
    	$('#fileupload-'+data_config.chat_id).fileupload({
    		...
    		add: function(e, data) {
                ...
                //console.log("uploading file1",e);
				var response = data.response();
                console.log("data", data.result.one.id);
                console.log("data", data.result.one.name);
                console.log("response", data.originalFiles[0]['type']);
				if (response != undefined && response.result != undefined && response.result.error == 'true' && response.result.error_msg != undefined) {
					alert(response.result.error_msg);
				} else {
                    var textArea = $("#CSChatMessage-"+data_config.chat_id);
                    var nickName = $("#messagesBlock-"+data_config.chat_id+" span.usr-tit").first().text();
                    var nickName1 = nickName.substring(9);
                   // console.log("nickName",nickName);
					lhinst.updateChatFiles(data_config.chat_id);
                    //console.log("uploading file2", "done");
                    var xmlhttp1 = new XMLHttpRequest();  
                    xmlhttp1.open("POST", "https://lab.intentico.ai/maya_multitenant_test/agentChatImage");
                    xmlhttp1.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
                    var data = JSON.stringify({
                      session_id:nickName1,
                      "imageUrl": "http://support.intentico.xyz/"+data.result.one.file_path+"/"+data.result.one.name,
                      "chat_id":data_config.chat_id,
                      "file_name":data.result.one.name,
                      "id":data.result.one.id,
                      "fileType":data.result.one.type
                    });
    ...
    lhinst.syncadmincall();
     ...
     //let's create arrays
       ...
       /* //If parent option is changed
        ...
        {
            //console.log("came here",array_list);
           // $("#child_selection").html(""); //reset child options
            $(array_list).each(function (i) { //populate child options 
                //console.log("value",array_list[i].value);
                //console.log($("#child_selection").append("<option value=\""+array_list[i].value+"\">"+array_list[i].display+"</option>"));
            });
        }
...
document.getElementById("subcategory").length = 0;
                ...
                var items = {
                "Accounts":["Accounts__Payment link shared","Accounts__Refund/Payment"],
                "Adhoc" : ["Adhoc_Adhoc"],
                "Customer Initiated" : ["pricing_discount_product","Account details modification","Cx wants to modify order","Duplicate Invoice request with","Order Inquiry within TAT","Requesting for Rescheduling","Return request change of Rx"],
                "Digidesk" : ["Delay in digitization","Request for Refund/Replacement"],
                "E Consultation" : ["Doctor consultation pending","Doctor Cancelled Order"] ,
                "FC Pharmacist" : ["FC modifying the quantity_SKU","Missing SKU_Wrong SKU","Order not packed by Pharmacist","Order status PFS_Post FC Assigned","Request for Refund Replacement","RX Not Returned","Pricing Mismatch"],
                "IPP Related" : ["Delay in procurement","NAP uploaded","Order modified due to PFS"],
                "Logistics_Delivery" : ["Damaged package delivered","Delay in delivery 3PL","Delivery delayed by Medlife","Delivery yet to be assigned","Return pick up not done","RX Not picked up","TSA Query","TSAresc_cancel wout Cx consent","wrong package"],
                "Marketing Related" : ["Coupon code_Discount related","Ecash related concern","External Wallet related inquiry","Medlife wallet related","Share and Earn","sub _unsub"],
                "New Order": ["creation","Reopen"],
                "Service Related": ["Call Transferred_Digi desk","Call Transferred_Escalation desk","Call Transferred to RED","Incomplete call_Callback required","Lab test related","Stop Medlife promotions"],
                "Franchise_customer" : ["Order_Status ","Product_discount_price"],
                "Others": ["Other__Others","Chat_Ended"]
...
for(var index in items) {
    ...
    console.log(items[index])
        ...
        var dropdown = document.getElementById("subcategory");
            ...
            dropdown[dropdown.length] = new Option(items[index][i], items[index][i]);
        }
    }
